...
| `needs-attention` | Claude가 응답을 완료했으나 사용자가 아직 확인하지 않음 |
| `inactive` | Claude 프로세스가 실행되지 않는 상태 |

`needs-attention`은 `busy → idle` 전환 시 자동 승격되며, 사용자가 해당 탭을 포커스하면 `idle`로 전환된다.
...
| --- | --- | --- |
| `busy` | `busy` | 스피너 |
| `needs-attention` | `needs-attention` | 보라색 점 (펄스) |
| `idle` | `idle` | 표시 없음 |
| `inactive` | `idle` | 표시 없음 |
...
| 경로 | setter | 특성 |
| --- | --- | --- |
| 로컬 (onSync) | `setCliState` | `busy→idle` 시 `needs-attention` 승격, `needs-attention→idle` 보호 |
| 서버 (status WS) | `syncAllFromServer`, `updateFromServer` | 서버가 authority, 직접 patch (승격/보호 없음) |
| 사용자 액션 | `dismissTab` | `needs-attention → idle` 전환 |

### `isCliIdle()` 헬퍼

`idle`과 `needs-attention` 모두 "작업 대기 중"을 의미하므로, idle-like 판단이 필요한 곳에서 사용:
...
### needs-attention 승격 로직

서버(`StatusManager.poll`, `StatusManager.updateTab`)와 클라이언트(`setCliState`)에서 동일하게 적용:

- `busy → idle` 전환 시 `needs-attention`으로 승격
- `needs-attention` 상태에서 `idle` 수신 시 무시 (보호)
- `dismissTab` 호출 시 `needs-attention → idle` 전환
...
[서버 폴링] status-manager.ts
  ...
  │
  └─ 상태 변경 감지 시 WebSocket 브로드캐스트 (needs-attention 승격 포함)
      │
      ▼
...
타임라인 WS 연결 → detectActiveSession()
  ...
  └─ timeline:append 메시지 → entries 추가
      ...
      └─ useTimeline onSync 콜백
          ...
          ├─ setCliState(tabId, cliState)    ← 여기서 needs-attention 승격
...
| 파일 | 설명 |
| --- | --- |
| `src/lib/status-manager.ts` | 폴링 엔진, `checkJsonlIdle`, needs-attention 승격, 상태 브로드캐스트 |
