...
- ✅ 셸 escaping/Hook 등록 축소/legacy 필드 정책/UUID 검증 등 세부 결정
- ✅ Phase 1 (코어) + Phase 2 (UI 1급 시민화) 범위 분리
- ✅ **2026-05-01 검토 결과 — 기존 예정 작업 범위(jsonl 파서 / 타임라인 / stats / session-history / observer)를 v19에 흡수, Phase 3/4로 재편**
- ✅ **2026-05-01 검토 결과 — Hook 실행 모델 정정** (`async=true` 가정 → `join_all` 병렬 + codex 본체가 await로 기다림). 실제 안전 근거는 "각 hook 별 프로세스 + stdin 격리"
- ⏭️ 구현 진입 가능. 자세한 Phase별 책임/공통화 정책은 `PRD.md` 본문 참조

## 작업 단계 요약

| Phase | 범위 | 결과물 |
| --- | --- | --- |
| **1** | Provider 코어 + hook 인프라 + 에러 표면화 | Codex가 purplemux 안에서 동작 (terminal 모드). 상태 감지/auto-resume/시스템 프롬프트 ✓ |
| **2** | UI 1급 시민화 | Codex 메뉴/아이콘/단축키/패널(타임라인 placeholder)/세션 목록/권한 응답 |
| **3** | jsonl 파서 + 타임라인 | Codex jsonl 파서 + ITimelineEntry 6개 신규 타입 + 컴포넌트 6개 + timeline-server provider 분기 → CodexPanel placeholder 제거 |
| **4** | stats / session-history / observer 통합 | `jsonl-parser.ts` 일반화 (token + rate_limits) + `~/.codex/sessions/` 디렉토리 스캔 + `attachWorkStateObserver` 정식 구독 |
| **2 (v19 외부)** | 추가 일반화 | `AgentPanel` 일반화, `ISessionListEntry` 일반화, 다중 provider 성능 측정, 세션 검색/필터링, `--add-dir` 매핑 |
...
| `attachWorkStateObserver?` | 예정 작업: hook 이벤트 변환 + jsonl tail 감시 |
...
2. **사용자 config 머지 로직**:
   ...
   - 4개 이벤트 각각 우리 entry + 사용자 entry 합친 배열 생성. **순서**: `[...ourEntries, ...userEntries]` — 우리 hook이 먼저(deterministic 출력 + display_order 메타 보존). **실행 모델 정정 (2026-05-01 검증)**: codex 소스(`codex-rs/hooks/src/engine/dispatcher.rs:88-101`)는 `futures::future::join_all` 사용해 hook 배열을 **병렬 실행** + codex 본체가 await로 모두 완료까지 기다림 (`HookExecutionMode::Sync`). 즉 머지 순서는 실행 순서에 영향 없음. **안전 근거**: 각 hook은 별 프로세스 + stdin 격리라 사용자 hook이 stdin 소비/실패해도 우리 hook 무관. **caveat — 우리 책임 밖**: 사용자 hook이 timeout 안에서도 느리면 codex 본체 진행 그만큼 지연됨. 우리 hook은 `curl POST localhost` 수십 ms라 codex에 부담 없음
...
Codex 탭이 사용자 입장에서 자연스럽게 보이게 하는 최소 UI 작업. jsonl 파서/타임라인은 예정 작업 유지.
...
2. `src/components/features/workspace/codex-panel.tsx`
   - **ClaudeCodePanel 거의 그대로 복사 + 차이점만 변경** (옵션 A 채택). 코드 중복은 예정 작업(v19 외부)에서 일반화 시 제거.
   - 차이점:
     - 헤더 아이콘: `CodexIcon`
...
4. `src/lib/providers/codex/session-list.ts` — `~/.codex/sessions/YYYY/MM/DD/*.jsonl` 스캔 + 첫 줄 SessionMeta 추출 (예정 작업 jsonl 파서 없이도 메타만 파싱)
   - **옵션 A 채택**: Claude session-list와 별도 모듈/컴포넌트. UI 재사용 없이 각자 자기 데이터 형식. 예정 작업(v19 외부)에서 `ISessionListEntry` 일반화 시점에 통합.
   - export 시그니처:
     ```ts
...
13. **PermissionRequest 응답 흐름** — 사용자가 UI에서 Yes/No 클릭 시 send-keys로 codex 응답.
    ...
    - Phase 2 매핑:
      - "Yes" → `y`
      - "No" → `n`
    - 예정 작업(v19 외부)에서 추가 옵션(`a`/`p`) UI 노출.
    - **caveat**: 사용자가 keymap 커스터마이즈한 경우(`~/.codex/config.toml`의 `[keymap.approval]` 변경) default 키가 다름. Phase 2는 default만 가정. 사용자 keymap 읽어 매핑은 예정 작업(v19 외부).
...
### Phase 2에 안 함 (Phase 3/4로 이관)
- jsonl 파싱 → 풀 타임라인 → **Phase 3 (PRD §11)**
- session-meta-cache codex 통합 → **Phase 4**
- session-history (notification-sheet) codex 지원 → **Phase 4**
- stats 페이지 codex 토큰 → **Phase 4**
- PreToolUse/PostToolUse → currentAction 추적 → 예정 작업
- `attachWorkStateObserver` 정식 통합 → **Phase 4**

## Phase 3 — jsonl 파서 + 타임라인 (v19 흡수)

**PRD §11 참조**. 핵심 결정:
- **공통화 (옵션 A)**: `ITimelineEntry` 타입 시스템 재사용 + 6개 신규 타입 추가 (`approval-request`, `exec-command-stream`, `web-search`, `mcp-tool-call`, `patch-apply`, `context-compacted`). 기존 14개 timeline 컴포넌트 12개 그대로 재사용 + 신규 6개 (1:1 매핑)
- **파서 분리**: `src/lib/session-parser-codex.ts` 신규 (~700-900줄). RolloutItem(`session_meta`/`response_item`/`event_msg`/`compacted`/`turn_context`) → `ITimelineEntry`. linear 구조라 트리 walking 없음
- **timeline-server provider 분기**: jsonl path로 provider 검출, 파서 라우팅. WebSocket protocol 변경 없음
- **분기 영역 (작음)**: `agent-group`은 Claude 전용 유지 (Codex sub-agent 없음), `thinking`은 Codex `encrypted_content` 케이스 분기 (summary만 표시), `assistant-message.usage`는 optional 유지 (Codex는 session-level token_count로 별도)

## Phase 4 — stats / session-history / observer 통합 (v19 흡수)

**PRD §11.6 참조**. 핵심 결정:
- **stats 일반화**: `src/lib/stats/jsonl-parser.ts` (Claude 전용) + 신규 `jsonl-parser-codex.ts` → 통합 aggregator에서 병합. 토큰 합산 방식 차이 흡수 (Claude 라인별 합산 vs Codex `token_count` event total)
- **Codex 전용 추가 노출**: `rate_limits.primary.used_percent`, `model_context_window`, `cached_input_tokens`, `reasoning_output_tokens`
- **session-history**: `claudeSessionId` 기반 식별을 `agentSessionId + providerId`로 일반화
- **observer 정식 통합**: `attachWorkStateObserver` Step 5 TODO 해소

## 예정 작업

각 항목이 자체로 큰 작업. v19 종료 후 별도 spec.

- **PreToolUse/PostToolUse hook 활용** — currentAction 갱신 (Phase 2에서는 미등록)
- **`AgentPanel` 일반화** — ClaudeCodePanel/CodexPanel 코드 중복 제거
- **`ISessionListEntry` 일반화** — Phase 2의 별도 session-list 모듈 통합
- **사용자 keymap 인지** — 사용자가 `~/.codex/config.toml`의 `[keymap.approval]` 변경한 경우 권한 응답 키 매핑 동적 처리. (Phase 2는 default `y`/`n` + 3초 timeout fallback 토스트로 대처)
- **다중 provider 동시 detection 성능** — 폴링 비용 측정 + 사이클 동적 조정
- **세션 검색/필터링** — 디렉토리 스캔 비용 ↑ 시 SQLite state DB 활용
- **`--add-dir` 매핑** — codex 멀티 디렉토리 옵션과 purplemux 워크스페이스 multiple directories 매핑
...
PreToolUse/PostToolUse는 예정 작업(v19 외부)에서 currentAction 추적이 필요할 때 추가.
...
3. **UI 메뉴 (Phase 2) — preflight 결과 따라 disabled**:
...
| 3 | pane content에 **composer 박스(`╭` AND `╰`) AND 마커(`›` U+203A 또는 `!`)**. codex composer의 두 줄 박스 + 입력 prompt indicator (`chat_composer.rs:4334-4351`). **2026-05-01 강화**: 단순 `›`/`!` substring 검색은 spaceship/oh-my-posh 일부 zsh 테마 + codex 시작 직전 zsh prompt 잔상 + splash/changelog의 `!` 등으로 false positive → false idle 전환 시 dead state 위험. 박스 문자(`╭`, `╰`)는 일반 shell prompt에서 거의 안 씀 | 중간 (`capturePaneAtWidth` 캡처) | 가장 정확 — composer 실제 렌더링 확인 |
...
if (provider && paneInfo?.pid && existing.cliState === 'inactive') {
  ...
  // Layer 3: pane content에 composer 박스 + 마커 (강화 — 박스 없으면 spaceship 등 false positive)
  const content = await capturePaneAtWidth(tab.sessionName, /* width */ 100);
  const hasBox = content && content.includes('╭') && content.includes('╰');
  const hasMarker = content && (content.includes('›') || content.includes('!'));
  if (!hasBox || !hasMarker) return;  // 다음 폴링 사이클 재시도
...
**Phase 2 추가 가능 (선택)**: 이벤트 기반 트리거(tmux 콘텐츠 변경 watch)로 latency↓. 우선 폴링 충분.
...
**폴링 vs 이벤트 기반**:
- Phase 1/2는 **폴링**(기본 1-2초 사이클). 충분히 반응 빠르고 단순.
- 이벤트 기반(tmux 콘텐츠 변경 watch)는 latency↓이지만 구현 복잡. 예정 작업 옵션.
...
**해결** — 폴링 사이클마다 일반 검사 + **ping-pong 방지 fallback (2026-05-01 추가)**:
```ts
// status-manager poll() 일반 사이클 (Phase 1)
...
if (!agentRunning
  ...
  // F1: Recent-launch grace 5초 — auto-resume 직후 codex 부팅 1-2초 윈도우 차단
  const recentLaunch = existing.lastResumeOrStartedAt
    && (now - existing.lastResumeOrStartedAt) < 5000;

  // F2: Pane title이 여전히 agent 형식 — codex가 잠깐 fork/exec 하는 순간 차단
  // (shell 복귀 시 title이 즉시 cmd|path 형식으로 변경되므로 정상 종료엔 영향 없음)
  const paneTitle = existing.paneTitle ?? '';
  const titleStillAgentLike = paneTitle && !/^[^|]+\|[^|]+$/.test(paneTitle);

  if (recentLaunch || titleStillAgentLike) {
    // skip — 잠깐 안 보일 뿐
  } else {
    // 진짜 종료 → inactive 복귀
...
`entry.lastResumeOrStartedAt` 신설. 갱신 시점:
- `auto-resume.ts`의 sendResumeKeys 직후
- SessionStart hook 수신 시 (synthetic + real)

비용: `paneTitle`은 이미 polling 사이클에서 확보되어 있음 → 추가 호출 없음.

Claude/Codex 모두 영향 (Claude도 `/exit` 후 같은 시나리오. 지금까지 별 문제 없었지만 명시적 처리 안 됨 + ping-pong 견고화 효과).

**회귀 검증 필요**:
- Claude의 정상 종료 후 새 명령 실행 흐름이 깨지지 않는지 (시나리오 S3)
- Codex auto-resume 직후 ping-pong 발생 안 하는지 (시나리오 S1, F1으로 차단)
- codex가 잠깐 fork/exec 하는 순간 false inactive 안 떨어지는지 (시나리오 S2, F2로 차단)
...
- `auto-resume.ts:73` (`await sendKeys(target.tmuxSession, resumeCmd)`)
  - **확인됨**: `sendKeys` 구현(`tmux.ts:328`)이 `tmux send-keys ... <command> Enter` — tmux가 두 인자를 **단일 호출 내 sequential PTY write로 보냄** (엄밀히 atomic은 아니지만 한 chunk로 도달 가능성 매우 높음, PIPE_BUF 한계 내). text+`\r`이 한 chunk로 codex 입력 파서에 도착 → paste burst 또는 줄바꿈 인지 가능. **2026-05-01 검증**: web-input-bar의 50ms 분리 패턴으로 분리 필요성 + Claude 영향 없음 모두 실증 → 무조건 도입 결정.
...
**Phase 2 정책**: 워크스페이스의 첫 번째 directory 기준 cwd 필터링 (Claude session-list 패턴과 동일). cwd 불일치 세션은 표시 안 함. 모두 보기 토글은 예정 작업(v19 외부).
...
### 미해결/Phase 1 범위 밖
- **PermissionRequest 응답 흐름** — Phase 2에서 default keymap 가정 (`y`/`n`) + 3초 timeout fallback 토스트로 처리 (PRD §9). 사용자 keymap 인지는 예정 작업
- **PreToolUse/PostToolUse hook 활용** — 현재 어휘에 없음. 등록은 하지만 status-manager에서 무시. 예정 작업(v19 외부)에서 currentAction 추적용으로 활용 가능.
- **SessionStart `source=clear` 처리** — 같은 탭의 새 session_id로 갱신하는 흐름. 모든 SessionStart에서 session_id 갱신 명세(PRD §6)로 자동 처리.
- **사용자 hook 명령 충돌 사례** — **2026-05-01 검증 완료**: codex 소스(`dispatcher.rs:88-101`)는 `join_all` 병렬 실행. 각 hook 별 프로세스 + stdin 격리라 우리 hook 영향 없음. 사용자 hook 지연 = codex 본체 지연(우리 책임 밖, caveat 문서화).
...
5. `/clear` → SessionStart(source=clear) → 새 `agentSessionId` + 이전 turn 메타(`agentSummary`/`lastUserMessage`/`lastAssistantMessage`) reset + 강제 idle 전환 + timeline `session-changed` (reason=`new-session-started`) + ContextRing token 0% reset
6. `codex resume <id>` 직접 입력 vs purplemux auto-resume 둘 다 정상 동작
7. 사용자가 자체 hook 등록한 상태(예: code-notify)에서 우리 hook 동시 실행 (병렬, 우리 hook 영향 없음)
8. **종료 흐름**: codex 탭에서 `/quit` 또는 Ctrl+D → process 종료 → shell 복귀 → tmux set-titles 자동 갱신(`zsh|/path`) → §5 F2 통과 안 함 → cliState='inactive' 자동 전환 → UI에 빈 상태 + 시작 안내. Claude도 `/exit` 후 동일 회귀 검증 (기존 잠재 버그 자연 해결)
9. 서버 재시작 → auto-resume → 셸 ready 타이밍(500ms) 충분한지 + §5 F1 grace 5초로 ping-pong 차단
