...
서버 재시작 시 비밀번호/토큰이 바뀌지 않도록 환경변수로 고정할 수 있습니다:

```bash
AUTH_PASSWORD=mypassword AUTH_TOKEN=mysecrettoken pnpm dev
```
...
AUTH_TOKEN=mysecrettoken123456789abcdef
```

- `AUTH_PASSWORD` + `AUTH_TOKEN` 둘 다 설정 → 해당 값 사용 (파일 생성/조회 스킵)
- 하나라도 미설정 → 기존대로 랜덤 생성
- 서버 로그에 `(env)` 표시로 고정 여부 확인
